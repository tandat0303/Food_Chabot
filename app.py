...
def meal_plan() -> tuple[Dict[str, Any], int]:
    try:
        data = request.get_json(silent=True)
        logger.debug("Received data for meal plan: %s", data)
        ...
        logger.info("Generating meal plan for user %s with ingredient: %s, diet: %s", user_id, ingredient, diet)

        meal_plan_result = suggest_meal_plan(user_id, ingredient, diet)
        logger.debug("Meal plan result: %s", meal_plan_result)
